# Pyodide runs here, in a Web Worker, so that a blocking input() or an infinite
# loop freezes only this worker thread, never the VS Code tab. stdin blocks on a
# synchronous long-poll to the gateway (allowed in a worker); the terminal feeds
# lines to that queue. The worker posts stdout/diags/etc back to the iframe,
# which relays them to the extension over the BroadcastChannel.
#
# pyodide-worker.js is the two dozen lines of JavaScript that compile this file
# and run it. Nothing here waits with await: LiveScript has no async functions,
# so the order of things is a promise chain.

pyodide = null
ready = null
black-ready = null
current-run-id = null

PY_SETUP = '''
import json
def _lc3_syntax_check(src):
    try:
        compile(src, '<student>', 'exec')
        return '[]'
    except SyntaxError as e:
        return json.dumps([{ 'line': e.lineno or 1, 'col': e.offset or 1,
                             'message': 'SyntaxError: ' + (e.msg or ''),
                             'severity': 'error' }])
'''

ensure-pyodide = ->
  return ready if ready
  ready := Promise.resolve!
    .then ->
      importScripts '/heavy/pyodide/pyodide.js'
      loadPyodide indexURL: '/heavy/pyodide/'
    .then (py) ->
      pyodide := py
      pyodide.runPython PY_SETUP
      pyodide
  ready

post = (m) -> postMessage m
out = (run-id, stream, data) -> post {t: 'out', runId: run-id, stream: stream, data: data}
status = (run-id, text) -> post {t: 'status', runId: run-id, text: text}

fetch-bundle = (path) ->
  fetch '/api/runbundle?path=' + encodeURIComponent(path), credentials: 'same-origin'
    .then (r) ->
      throw new Error "cannot load your files (#{r.status})" unless r.ok
      r.json!

fetch-file = (path) ->
  fetch '/api/fs/read?path=' + encodeURIComponent(path), credentials: 'same-origin'
    .then (r) ->
      throw new Error "cannot read file (#{r.status})" unless r.ok
      r.text!

b64-to-bytes = (b64) -> Uint8Array.from atob(b64), (c) -> c.charCodeAt 0

# Blocking read of one line: a synchronous long-poll on this worker thread.
py-stdin = ->
  return null unless current-run-id
  x = new XMLHttpRequest!
  x.open 'GET', '/api/run/stdin?run=' + encodeURIComponent(current-run-id), false
  try
    x.send null
  catch
    return null
  if x.status is 200 then x.responseText else null

py-runner = (entry) -> """
import sys, runpy
for _m in list(sys.modules):
    _f = getattr(sys.modules.get(_m), '__file__', '') or ''
    if _f.startswith('/proj/'):
        del sys.modules[_m]
if '/proj' not in sys.path:
    sys.path.insert(0, '/proj')
sys.argv = [#{JSON.stringify entry}]
runpy.run_path('/proj/' + #{JSON.stringify entry}, run_name='__main__')
"""

reset-proj = ->
  try
    pyodide.runPython "import shutil; shutil.rmtree('/proj', ignore_errors=True)"
  try
    pyodide.FS.mkdir '/proj'

write-proj = (files) ->
  for own rel, b64 of files
    parts = rel.split '/'
    dir = '/proj'
    for part in parts.slice 0, -1
      dir += '/' + part
      try
        pyodide.FS.mkdir dir
    pyodide.FS.writeFile '/proj/' + rel, b64-to-bytes b64

clean-traceback = (text) ->
  lines = text.split '\n'
  i = lines.findIndex (l) -> l.includes '/proj/'
  return text.replace /\/proj\//g, '' if i < 0
  ('Traceback (most recent call last):\n' + lines.slice(i).join '\n').replace /\/proj\//g, ''

parse-traceback = (text, entry) ->
  re = new RegExp 'File "/proj/' + entry.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '", line (\\d+)', 'g'
  last = null
  loop
    m = re.exec text
    break unless m?
    last = parseInt m[1], 10
  lines = text.trim!.split '\n'
  if last
    [{line: last, col: 1, message: (lines[lines.length - 1] or 'error'), severity: 'error'}]
  else
    []

# Wire the student's program to the terminal: output byte for byte (not batched
# on newline, so a prompt written without one appears before the read), and
# stdin to the blocking long-poll.
attach-streams = (run-id) ->
  dec = new TextDecoder!
  pyodide.setStdout write: (buf) ->
    out run-id, 'stdout', dec.decode buf
    buf.length
  pyodide.setStderr write: (buf) ->
    out run-id, 'stderr', dec.decode buf
    buf.length
  pyodide.setStdin stdin: py-stdin, autoEOF: true

# Load the student's folder into /proj and hand back the file to start at.
prepare = (run-id, path) ->
  status run-id, 'starting Python'
  fetch-bundle path .then (bundle) ->
    reset-proj!
    write-proj bundle.files
    entry = bundle.entry or 'main.py'
    attach-streams run-id
    current-run-id := run-id
    entry

# What both run and debug do when the program is over: report where it broke,
# if it broke, and that it is no longer running.
finish = (run-id, path, diags) ->
  current-run-id := null
  post {t: 'diags', path: path, diags: diags}
  post {t: 'exit', runId: run-id, code: (if diags.length then 1 else 0)}

run-python = (run-id, path) ->
  ensure-pyodide!
    .then -> prepare run-id, path
    .then (entry) ->
      status run-id, 'running'
      pyodide.runPythonAsync py-runner entry
        .then -> []
        .catch (e) ->
          msg = String (e?.message or e)
          out run-id, 'stderr', (clean-traceback msg) + '\n'
          parse-traceback msg, entry
    .then (diags) -> finish run-id, path, diags

# ---------------------------------- debug -----------------------------------
# Debugging is a normal run with a trace hook attached. When the hook decides to
# stop, it posts where the program is to the gateway and blocks on that same
# request until the editor answers with a step or a continue. It has to be a
# synchronous request for the same reason input() is one: this thread is inside
# the student's program and cannot come back to an event loop to receive a
# message.

# Post one event, block, return the editor's next command (as JSON text).
# Python calls this; a failure reads as "stop", which unwinds the program.
self.lc3DebugAsk = (payload) ->
  return '{"cmd":"stop"}' unless current-run-id
  x = new XMLHttpRequest!
  x.open 'POST', '/api/run/debug/pause?run=' + encodeURIComponent(current-run-id), false
  x.setRequestHeader 'Content-Type', 'application/json'
  try
    x.send payload
  catch
    return '{"cmd":"stop"}'
  return '{"cmd":"stop"}' if x.status isnt 200 or not x.responseText
  x.responseText

PY_DEBUGGER = '''
import json, sys, types

class _Lc3Stop(Exception):
    pass

class _Lc3Debugger:
    # Values are shown as one line of repr, with containers opened two levels
    # down. Deeper than that is a data structure the student should print, not
    # a variable they are watching.
    MAX_ITEMS = 50
    MAX_DEPTH = 2

    def __init__(self, breakpoints, ask):
        self.ask = ask
        self.set_breakpoints(breakpoints)
        self.mode = 'run'
        self.stop_depth = 0
        self.frames = {}

    def set_breakpoints(self, mapping):
        # Keys arrive relative to the program's folder; the copy runs in /proj.
        self.bps = {}
        for rel, lines in (mapping or {}).items():
            self.bps['/proj/' + rel.lstrip('/')] = set(lines or ())

    # ---- values ----

    def repr_of(self, value):
        try:
            text = repr(value)
        except Exception:
            text = '<unreadable>'
        return text if len(text) <= 200 else text[:197] + '...'

    def children(self, value, depth):
        if depth >= self.MAX_DEPTH:
            return []
        out = []
        if isinstance(value, dict):
            for k, v in list(value.items())[:self.MAX_ITEMS]:
                out.append(self.variable(str(k), v, depth + 1))
        elif isinstance(value, (list, tuple, set, frozenset)):
            for i, v in enumerate(list(value)[:self.MAX_ITEMS]):
                out.append(self.variable('[' + str(i) + ']', v, depth + 1))
        elif hasattr(value, '__dict__') and not isinstance(value, type):
            for k, v in list(vars(value).items())[:self.MAX_ITEMS]:
                if not k.startswith('_'):
                    out.append(self.variable(k, v, depth + 1))
        return out

    def variable(self, name, value, depth=0):
        return {
            'name': name,
            'value': self.repr_of(value),
            'type': type(value).__name__,
            'children': self.children(value, depth),
        }

    def scope(self, mapping, is_globals):
        out = []
        for name, value in list(mapping.items())[:200]:
            if name.startswith('__') and name.endswith('__'):
                continue
            if is_globals and isinstance(value, types.ModuleType):
                continue
            out.append(self.variable(name, value))
        return out

    # ---- stopping ----

    def depth(self, frame):
        n = 0
        while frame is not None:
            n += 1
            frame = frame.f_back
        return n

    def state(self, frame, reason):
        frames = []
        self.frames = {}
        fid = 1
        f = frame
        while f is not None and f.f_code.co_filename.startswith('/proj/'):
            frames.append({
                'id': fid,
                'name': f.f_code.co_name if f.f_code.co_name != '<module>' else 'module',
                'line': f.f_lineno,
                'path': f.f_code.co_filename[len('/proj/'):],
                'locals': self.scope(f.f_locals, False),
                'globals': self.scope(f.f_globals, True),
            })
            self.frames[fid] = f
            fid += 1
            f = f.f_back
        return {'t': 'stopped', 'reason': reason, 'frames': frames}

    def evaluate(self, expression, frame_id):
        frame = self.frames.get(frame_id) or self.frames.get(1)
        if frame is None:
            return None
        try:
            value = eval(expression, frame.f_globals, frame.f_locals)
        except Exception as exc:
            return {'name': expression, 'value': type(exc).__name__ + ': ' + str(exc),
                    'type': 'error', 'children': []}
        return self.variable(expression, value)

    def pause(self, frame, reason):
        message = self.state(frame, reason)
        while True:
            try:
                command = json.loads(self.ask(json.dumps(message) if message else ''))
            except Exception:
                raise _Lc3Stop()
            what = command.get('cmd')
            if what == 'wait':
                # Nobody has pressed anything yet. Ask again with nothing to
                # say, so the request never sits long enough to be timed out
                # somewhere between here and the gateway.
                message = None
                continue
            if what == 'evaluate':
                message = {'t': 'eval', 'reqId': command.get('reqId'),
                           'value': self.evaluate(command.get('expression') or '',
                                                  command.get('frameId'))}
                continue
            if what == 'breakpoints':
                self.set_breakpoints(command.get('breakpoints'))
                message = {'t': 'ack'}
                continue
            if what == 'stop':
                raise _Lc3Stop()
            if what == 'next':
                self.mode, self.stop_depth = 'next', self.depth(frame)
            elif what == 'stepIn':
                self.mode = 'stepIn'
            elif what == 'stepOut':
                self.mode, self.stop_depth = 'stepOut', self.depth(frame)
            else:
                self.mode = 'run'
            return

    def trace(self, frame, event, arg):
        # Only the student's own files are worth stepping through; the standard
        # library and Pyodide's own machinery are not what is being debugged.
        if not frame.f_code.co_filename.startswith('/proj/'):
            return None
        if event != 'line':
            return self.trace
        reason = None
        if frame.f_lineno in self.bps.get(frame.f_code.co_filename, ()):
            reason = 'breakpoint'
        elif self.mode == 'stepIn':
            reason = 'step'
        elif self.mode == 'next' and self.depth(frame) <= self.stop_depth:
            reason = 'step'
        elif self.mode == 'stepOut' and self.depth(frame) < self.stop_depth:
            reason = 'step'
        if reason:
            self.pause(frame, reason)
        return self.trace

def _lc3_debug(entry, breakpoints_json, ask):
    import runpy
    debugger = _Lc3Debugger(json.loads(breakpoints_json), ask)
    for name in list(sys.modules):
        path = getattr(sys.modules.get(name), '__file__', '') or ''
        if path.startswith('/proj/'):
            del sys.modules[name]
    if '/proj' not in sys.path:
        sys.path.insert(0, '/proj')
    sys.argv = [entry]
    sys.settrace(debugger.trace)
    try:
        runpy.run_path('/proj/' + entry, run_name='__main__')
    except _Lc3Stop:
        return 'stopped'
    finally:
        sys.settrace(None)
    return 'finished'
'''

debug-python = (run-id, path, breakpoints) ->
  ensure-pyodide!
    .then -> prepare run-id, path
    .then (entry) ->
      status run-id, 'debugging'
      diags = []
      try
        pyodide.runPython PY_DEBUGGER
        run = pyodide.globals.get '_lc3_debug'
        stopped = run(entry, JSON.stringify(breakpoints or {}), self.lc3DebugAsk) is 'stopped'
        status run-id, 'stopped by the debugger' if stopped
      catch e
        msg = String (e?.message or e)
        out run-id, 'stderr', (clean-traceback msg) + '\n'
        diags = parse-traceback msg, entry
      finish run-id, path, diags

check-python = (path) ->
  ensure-pyodide!
    .then -> fetch-file path
    .then (src) ->
      json = pyodide.globals.get('_lc3_syntax_check') src
      post {t: 'diags', path: path, diags: JSON.parse json}
    .catch -> void  # best-effort

ensure-black = ->
  return black-ready if black-ready
  black-ready := ensure-pyodide!
    .then -> pyodide.loadPackage 'micropip'
    .then -> fetch '/runtime/pyfmt/wheels.json'
    .then (r) -> r.json!
    .then (wheels) ->
      micropip = pyodide.pyimport 'micropip'
      micropip.install wheels.map (wname) -> self.location.origin + '/runtime/pyfmt/' + wname
    .then ->
      pyodide.runPython 'import black'
      true
  black-ready

format-python = (req-id, src) ->
  ensure-black!
    .then ->
      pyodide.globals.set '__lc3_src', src
      pyodide.runPython 'black.format_str(__lc3_src, mode=black.Mode())'
    .catch -> src
    .then (text) -> post {t: 'formatted', reqId: req-id, text: text}

self.onmessage = (e) ->
  m = e.data or {}
  work = switch m.cmd
  | 'run' => run-python m.runId, m.path
  | 'debug' => debug-python m.runId, m.path, m.breakpoints
  | 'check' => check-python m.path
  | 'format' => format-python m.reqId, m.text
  | 'preload' => ensure-pyodide!.then -> post {t: 'ready', lang: 'python'}
  | otherwise => null
  work?.catch (err) ->
    if m.runId
      out m.runId, 'stderr', String((err?.message or err)) + '\n'
      post {t: 'exit', runId: m.runId, code: 1}
